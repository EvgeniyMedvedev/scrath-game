...
import org.scrathgame.components.matchers.CombinationMatcher;
import org.scrathgame.components.matchers.MatchingResult;
import org.scrathgame.models.Config;
import org.scrathgame.models.Symbol;
...
public class GameBoardAnalyzer {
    ...
    private BigDecimal applyBasicSymbols(Map<String, BigDecimal> rewardMultipliers, BigDecimal reward) {
        config.symbols().put(null, new Symbol(BigDecimal.ONE,null, null, null));
        BigDecimal rewardMultiplier = rewardMultipliers.entrySet().stream()
                .map(e -> config.symbols().get(e.getKey()).rewardMultiplier()
                        .multiply(e.getValue()))
